...
def test_dispatch_get_and_write_use_same_user_scoped_redirection(
    multi_user_repo: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _set_env_user(monkeypatch, "alice")
    ...
    write_result = _dispatch_tool(
        config,
        "memory_write",
        {"path": "memory-bank/activeContext.md", "content": "# Alice Dispatch\n", "backup": False},
    )
    read_result = _dispatch_tool(config, "memory_get", {"path": "memory-bank/activeContext.md"})
