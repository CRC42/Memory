...
def test_dispatch_governance_tools(repo: Path) -> None:
    config = load_config(repo)
    candidate = memory_write_record(
        config,
        content_markdown="# Dispatch Candidate\n\nDispatch should expose governance tools.\n",
        record_kind="rule_candidate",
        scope="personal",
        author="alice",
        tags=["mcp"],
    ...
    validated = _dispatch_tool(
        config,
        "memory_validate_candidate",
        {"record_id": candidate["id"], "validated_by": "lead"},
    )
    published = _dispatch_tool(
        config,
        "memory_publish_candidate",
        {"record_id": candidate["id"], "published_by": "owner"},
    )
    archived = _dispatch_tool(
        config,
        "memory_archive_record",
        {"record_id": candidate["id"], "reason": "superseded"},
    )
...
def test_build_tools_includes_governance_tools(repo: Path) -> None:
    ...
    assert "memory_publish_candidate" in tool_names
    assert "memory_archive_record" in tool_names
