...
def test_dispatch_rebuild_index_and_search_records(repo: Path) -> None:
    config = load_config(repo)
    memory_write_record(
        config,
    ...
    rebuild = _dispatch_tool(config, "memory_rebuild_index", {})
    search = _dispatch_tool(config, "memory_search_records", {"query": "indexing"})
