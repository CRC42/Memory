...
def test_cli_rebuilds_all_key_documents_with_deterministic_renderer(tmp_path: Path, capsys) -> None:
    ...
    rc, payload = _run_cli(
        ["--root", str(repo), "rebuild-key-docs", "--renderer", "deterministic"],
        capsys,
    )
    assert rc == 0, payload
    assert payload.get("ok") is True
    written = payload.get("written") or {}
    # All four key documents should have been written.
    assert set(written.keys()) == {"activeContext", "progress", "techContext", "systemPatterns"}
...
def test_cli_returns_error_when_embedding_renderer_disabled(tmp_path: Path, capsys) -> None:
    repo = _seed_repo(tmp_path)
    rc, payload = _run_cli(
        ["--root", str(repo), "rebuild-key-docs", "--renderer", "embedding"],
        capsys,
    )
    assert rc == 1
