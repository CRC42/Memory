...
def test_dispatch_exposes_runtime_maintenance_tools(repo: Path) -> None:
    config = load_config(repo)
    record = memory_write_record(
        config,
        content_markdown="# Dispatch Maintenance\n\nMaintenance tools should be callable.\n",
        record_kind="archive_record",
        scope="archive",
        status="archived",
        author="alice",
    ...
    health = _dispatch_tool(config, "memory_health_check", {})
    migrate = _dispatch_tool(config, "memory_migrate_records", {"target_schema_version": "1.0"})
    update = _dispatch_tool(config, "memory_update_index", {"paths": [record["path"]]})
    delete = _dispatch_tool(config, "memory_delete_record", {"record_id": record["id"], "reason": "dispatch"})
...
def test_build_tools_includes_runtime_maintenance_tools(repo: Path) -> None:
    ...
    assert "memory_migrate_records" in tool_names
    assert "memory_update_index" in tool_names
    assert "memory_delete_record" in tool_names
