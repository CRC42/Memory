...
def test_list_conflicts_reports_open_and_missing_edges(repo: Path) -> None:
    ...
    first_path = repo / first["path"]
    metadata, body = parse_record_markdown(first_path.read_text(encoding="utf-8"))
    metadata["conflicts_with"] = [second["id"]]
    first_path.write_text(render_record_markdown(metadata, body), encoding="utf-8")
...
def test_dispatch_exposes_p3_lineage_tools(repo: Path) -> None:
    ...
    observation = _dispatch_tool(
        ...
        "memory_record_observation",
        {
            "content_markdown": "# Dispatch Observation\n\nObservation through MCP dispatch.\n",
            "tags": ["mcp"],
            "module_names": ["MemoryServer"],
        },
    )
    linked = _dispatch_tool(
        config,
        "memory_link_artifact",
        {"record_id": observation["id"], "plugin_names": ["ProjectMemoryMCP"]},
    )
    traced = _dispatch_tool(config, "memory_trace_lineage", {"record_id": observation["id"]})

    assert "memory_record_observation" in tool_names
    assert "memory_link_artifact" in tool_names
    assert "memory_trace_lineage" in tool_names
...
def test_dispatch_context_lists_conflicts(repo: Path) -> None:
    ...
    result = _dispatch_tool(config, "memory_context", {"operation": "list_conflicts"})
