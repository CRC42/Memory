...
# chain when neither ``.vscode/settings.json`` nor env vars supply a
...
_SETUP_HINT = (
    "Set 'memory-mcp.userName' in .vscode/settings.json or run: "
    "powershell -Command \"$s = '.vscode/settings.json'; if (-not (Test-Path $s)) "
    "{ New-Item -ItemType Directory -Force -Path .vscode | Out-Null; "
    "'{}' | Set-Content $s }; "
    "$j = Get-Content $s -Raw | ConvertFrom-Json; "
    "$j | Add-Member -NotePropertyName 'memory-mcp.userName' -NotePropertyValue '<your-id>' -Force; "
    ...
    if is_ambiguous_user(name):
        return {
            ...
            "message": (
                ...
                "'memory-mcp.userName' in .vscode/settings.json to a personal id."
            ),
            "setup_hint": _SETUP_HINT,
        }
    return None
