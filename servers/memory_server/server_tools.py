            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "renderer": {
                        ...
                        "description": "Used by operation=rebuild_key_documents. Tiers: 'deterministic' (always available), 'llm' (P4-C, requires LLM client), 'embedding' (P5 Phase 2b, requires embeddings.enabled + built vector index), 'auto' (walks key_documents.prefer_order with fallback).",
                    },
                    "rewrite_query": {
                        "type": "boolean",
                        "default": False,
                        "description": "v0.10.0 — opt-in: ask the LLM to propose recall-friendly variants of `query` (synonyms / abbreviations / language switches) before ranking. Variants only broaden FTS recall; the deterministic ranker still picks the final order. Disabled by default; honours the unified runner's timeout/budget/fallback envelope.",
                    },
                    "rewrite_max_variants": {
                        "type": "integer",
                        "minimum": 1,
                        "maximum": 8,
                        "default": 3,
                        "description": "Maximum number of query variants to request from the LLM (capped at 8).",
                    },
                    "rewrite_context_hint": {
                        "type": "string",
                        "description": "Optional short domain hint appended to the rewrite prompt so variants can use the right vocabulary (e.g. project name, active task).",
                    },
                    "narrative": {
                        "type": "boolean",
                        "default": False,
                        "description": "v0.10.0 — opt-in for operation=compile with target ∈ {weekly_snapshot, monthly_snapshot}: prepend an LLM-generated executive summary to the deterministic snapshot body. Additive only — re-running without this flag produces an identical body.",
                    },
                },
                "additionalProperties": False,
            },
