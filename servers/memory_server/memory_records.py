...
ALLOWED_SCOPES = {"personal", "shared", "local", "archive"}
...
def _target_path(record_id: str, record_kind: str, scope: str, status: str, author: str) -> str:
    ...
    if scope == "shared" or status == "published" or record_kind == "system_rule":
...
def memory_write_record(
    ...
    *,
    ...
    normalized_source_refs = [str(ref).strip() for ref in (source_refs or []) if str(ref).strip()]
